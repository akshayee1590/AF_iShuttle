...
public class CategoryPage extends LoadableComponent<CategoryPage> {
	...
	/**
	 * 
	 * Constructor class for Category Page Here we initializing the driver for page
			 ...
			 if (!isPageLoaded) {
		        ...
		        try {
		            (new WebDriverWait(driver, 20).pollingEvery(200,
		                    ...
		                    .until(ExpectedConditions.urlContains("configure/category"));
	...
	public void searchCategory(String searchText) {
		 this.test1.log(Status.INFO,"Searching a Category by Search text :- " +searchText);
		searchBox.clear();
		searchBox.sendKeys(searchText);
		 this.test1.log(Status.PASS,"Searched a Category by Search text :- " +searchText);
	}
	 ...
	 * 
	 */
	public boolean verifySearchCategory(String searchText) {
		boolean flag = false;
